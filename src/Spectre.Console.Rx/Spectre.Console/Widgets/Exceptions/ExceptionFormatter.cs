...
internal static class ExceptionFormatter
{
    ...
    private static Grid GetStackFrames(Exception ex, ExceptionSettings settings)
    {
        ...
        grid.AddColumn(new GridColumn().PadLeft(2).PadRight(0).NoWrap())
            .AddColumn(new GridColumn().PadLeft(1).PadRight(0));
    ...
    private static bool ShowInStackTrace(StackFrame frame)
    {
        ...
        if (mb == null || (mb.MethodImplementationFlags & MethodImplAttributes.AggressiveInlining) != 0)
        {
            return false;
        }
...
#if NET6_0_OR_GREATER
            builder.Append('<')
                .AppendJoin(",", method.GetGenericArguments().Select(t => t.Name))
                .Append('>');
#else
            builder.Append('<')
                .Append(string.Join(",", method.GetGenericArguments().Select(t => t.Name)))
                .Append('>');
#endif
        }
