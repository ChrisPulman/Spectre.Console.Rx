...
internal static class TableRenderer
{
    ...
    public static List<Segment> Render(TableRendererContext context, List<int> columnWidths)
    {
        ...
        foreach (var (index, isFirstRow, isLastRow, row) in context.Rows.Enumerate())
        {
            ...
            // Show row separator?
            if (context.Border.SupportsRowSeparator && context.ShowRowSeparators && !isFirstRow && !isLastRow)
            {
                ...
                if (!isRenderingFooter)
                {
                    var separator =
                        Aligner.Align(
                            context.Border.GetColumnRow(TablePart.RowSeparator, columnWidths, context.Columns), context.Alignment, context.MaxWidth);
                    result.Add(new Segment(separator, context.BorderStyle));
                    result.Add(Segment.LineBreak);
                }
            }
            ...
            {
                var separator =
                    Aligner.Align(context.Border.GetColumnRow(TablePart.Bottom, columnWidths, context.Columns), context.Alignment, context.MaxWidth);
                result.Add(new Segment(separator, context.BorderStyle));
                result.Add(Segment.LineBreak);
            }
        }
